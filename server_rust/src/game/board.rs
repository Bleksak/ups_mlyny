...
impl Board {
    ...
    pub fn get(&self, index: usize) -> Result<Color, GameError> {
        Ok(*self
            ...
            .unwrap()
            .get(index)
            .ok_or(GameError::BadPosition)?)
    }

    /// Checks whether mill was formed vertically
    pub fn check_mill_vertical(
        &self,
        start: usize,
        guard: Option<MutexGuard<[Color; 24]>>,
    ) -> bool {
    ...
    pub fn check_draw(&self, last_color: Color) -> bool {
        ...
        let color = if last_color == Color::Red {
            Color::Blue
        } else {
            Color::Red
        };
    ...
    pub fn mmove(&self, (from, to): (usize, usize), jump_allowed: bool) -> Result<(), GameError> {
        ...
        if NEIGHBORS.len() > from && NEIGHBORS.len() > to {
            ...
            if !jump_allowed
                && (!NEIGHBORS[from].0.contains(&to) && !NEIGHBORS[from].1.contains(&to))
            {
